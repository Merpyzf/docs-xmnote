...
> 💡 我们更推荐你通过纸间书摘在 KOReader 中的高亮导出插件来完成笔记导出。
...
![Koodo 导出笔记的操作说明](https://doc-1252413502.cos.ap-nanjing.myqcloud.com/SCR-20250118-tfob.png)

如上图所示，您可以选择按单本书籍导出笔记，也可以一次性导出所有笔记或高亮内容。需要注意以下几点：

1. 单本书籍的「导出笔记」导出的 CSV 文件会包含该书籍的所有内容，包括高亮和笔记。
2. “导出全部” 的差异：1.“导出所有笔记”：仅包含有笔记记录的书摘，不包含高亮内容。2.“导出所有高亮”：仅导出书籍中的高亮内容，不包含笔记记录。

因此，如果您需要导出一本书的全部内容，可以直接使用单本书籍的「导出笔记」。而在导出全部书籍内容时，需要分别执行「导出所有笔记」和「导出所有高亮」操作，以确保完整性。

在获取到 CSV 文件后，您可以将其拷贝到手机中。完成后，按照以下步骤操作即可完成导入：

1. 打开 纸间书摘，进入书摘导入功能，选择“从文件导入”。
2. 在导入界面中，找到 Koodo Reader 导入，然后点击进入。
3. 选择已保存到手机的 CSV 文件，即可完成导入操作。

如果需要同时导入多个文件，可以在文件选择界面通过「长按文件」，一次性选择多个 CSV 文件，方便快速完成导入。操作流程如下图所示：

![Koodo 导入笔记的操作说明](https://doc-1252413502.cos.ap-nanjing.myqcloud.com/SCR-20250118-tlwi.png)
